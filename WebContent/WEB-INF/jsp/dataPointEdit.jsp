...
<script>
				...
				try {
					...
					for (var range in properties.eventTextRenderer.rangeEventValues) {
						for (let i = 0; i < thisEventRangeValues.length; i++) {
							let range_from = parseFloat(thisEventRangeValues[i].from) === parseFloat(properties.eventTextRenderer.rangeEventValues[range].from);
							let range_to = parseFloat(thisEventRangeValues[i].to) === parseFloat(properties.eventTextRenderer.rangeEventValues[range].to);
							if(range_from && range_to) {
								eventTextRendererEditor.removeRangeEventValue(
									parseFloat(properties.eventTextRenderer.rangeEventValues[range].from),
				...
				try {
					...
					for (var range in properties.textRenderer.rangeValues) {
						for (let i = 0; i < thisRangeValues.length; i++) {
							let if_from = parseFloat(thisRangeValues[i].from) === parseFloat(properties.textRenderer.rangeValues[range].from) &&
							let if_to = parseFloat(thisRangeValues[i].to) === parseFloat(properties.textRenderer.rangeValues[range].to))
							if (if_from && if_to) {
								textRendererEditor.removeRangeValue(
									parseFloat(properties.textRenderer.rangeValues[range].from),
									parseFloat(properties.textRenderer.rangeValues[range].to)
								);
							}
						}
	...
	function confirmText(properties) {
		...
		let logtype = properties.loggingType;
		let case_a = logtype == 1;
		let case_b = logtype == 2 || logtype == 3 || logtype == 5;
		let case_c = logtype == 4;
		if (case_a) {
